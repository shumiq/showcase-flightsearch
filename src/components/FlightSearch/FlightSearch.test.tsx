...
describe('FlightSearch', () => {
  ...
  it('calls onSearch with correct data when form is filled (desktop)', async () => {
    ...
    const fromSelect = screen.getAllByLabelText('From')[0] as HTMLSelectElement;
    const toSelect = screen.getAllByLabelText('To')[0] as HTMLSelectElement;
    const dateInput = screen.getAllByLabelText('Departure Date')[0] as HTMLInputElement;

    await user.selectOptions(fromSelect, 'BKK');
    await user.selectOptions(toSelect, 'CNX');
